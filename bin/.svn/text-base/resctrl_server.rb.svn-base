...
class ResCtrlServerSoapDriver < SOAP::RPC::StandaloneServer
  ...
  def initialize(service,namespace,address,port,archive_level=nil,archive_path=nil)
    ...
    servant = ResCtrlService::new(archive_level,archive_path)
...
class ResCtrlServerXMLRPCDriver
    def initialize(service,address,port,archive_level=nil,archive_path=nil)
        @address = address
        @port = port
        @servant = ResCtrlService::new(archive_level,archive_path)
...
class ResCtrlServer
	def initialize(service,namespace,address,port,archive_level=nil,archive_path=nil)
		if $RMI == 'soap'
			@rctrl_server = ResCtrlServerSoapDriver.new(service,namespace,address,port,archive_level,archive_path)
		elsif $RMI == 'xmlrpc'
			@rctrl_server = ResCtrlServerXMLRPCDriver.new(service,address,port,archive_level,archive_path)
		else
			$RMI = 'drb'
			@rctrl_server = ResCtrlServerDrbDriver.new(service,address,port,archive_level,archive_path)
		end
	end
...
if $0 == __FILE__
  ...
  opts.parse(ARGV)
  server = ResCtrlServer.new('resctrl', NS, '0.0.0.0', port, archive_level, archive_directory)
  trap(:INT) do 
    server.shutdown
  end
  server.start
