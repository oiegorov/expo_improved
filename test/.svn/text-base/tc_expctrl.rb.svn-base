...
server1 = ExpCtrlServer.new('expctrl', NS, '0.0.0.0', PORT)
trap(:INT) do 
  server1.shutdown
  exit
end
...
server2 = ExpCtrlServer.new('expctrl', NS, '0.0.0.0', PORT2)
trap(:INT) do
  server2.shutdown
  exit
end
...
class ExpCtrlTest < Test::Unit::TestCase
  ...
  def test_open_close_experiment
    client = ExpCtrlClient::new("http://localhost:#{PORT}")
  ...
  def test_recursive_command_1
    ...
    params1 = RecursiveCommandParameters::new("http://localhost:#{PORT}",'command',params2)
    result = client.recursive_command("http://localhost:#{PORT2}", 'recursive_command', params1)
  ...
  def test_recursive_command_2
    ...
    params1 = RecursiveCommandParameters::new("http://localhost:#{PORT}",'command',params2)
    begin
      response = client.recursive_command("http://localhost:#{PORT2+1}", 'recursive_command', params1)
      assert( false )
    rescue => details
      assert(true)
    end
...
'
  ...
  def test_command_delete
   client = RCtrlClient::new("http://localhost:#{PORT}")
   result = client.asynchronous_command("sleep 5")
