  ...
  def generic_command(params)
    ...
    if params["ruby_command"] then
      ...
      if params["interactive"] then
        ...
        #----here we just initialize a lot of instance variables with nil/false. The only important one is c.cmd
        #    containing our command line
    ...
    if params["delayed"] or params["asynchronous"] then
      #----on_exit function is in lib/cmdctrl/commands/command_bufferer.rb
      #    it just assigns the block to @on_exit_block
    ...
    #----assign a unique id for our command c
    ...
    if not params["delayed"] then
      #----remember the time when this command was started
      c.set_start_time
      #----to see run() method go to expo/lib/cmdctrl/commands/command_bufferer.rb:56
      #    actually runs "cmd" command, using fork from the main pgm and exec(cmd)
      c.cmd.run
      #----if our cmd is asynchronous we don't wait for its termination
      #    and continue execution of main process
      if not params["asynchronous"] then
        #----if our command is synchronous - we must wait for the
        #    previous command to finish before continue execution to
        #    wait we use Process::waitpid2(@pid) function
        c.cmd.wait
        c.set_end_time
        c.archive("#{@save_directory}/#{response["command_number"]}") if @archive == 1
    ...
    #----in case of asynch cmd response will contain only cmd id number
    return response
