...
class Oargrid2Reservation
  ...
  def make_reservation_command_line(parameters)
    ...
    command_line += " -t #{parameters.t}" if parameters.t
  ...
  def get_misc_data(stdout)
    misc_data = Hash::new
    key = stdout.scan(/SSH KEY : (.*)/).first
    misc_data["SSH KEY"] = key.first if key
    return misc_data
  end
