...
class ExpCtrlClient
    ...
    #----go to line 49 of this file to create a new ExpCtrlClientNoneDriver object
	...
	def method_missing(method_id, *args)
    #----we call method_id method on @expctrl_client object
    #----this object is of class ExpCtrlClientNoneDriver which derives
    #----from ExpCtrlService. This class has this method, so we call it
...
#  end
  ...
  def open_experiment( experiment_number = nil )
    ...
    #----we go here
      ...
      #----here we call method 'creat_experiment' which doesn't exist
      #----for this class. So we go to the method_missing method :)
      result = create_experiment
      #----yeeey! now we have a unique experiment number!
      @experiment_number = result["experiment_number"]
    end
    #----again, we just create a new hash and initialize
    #----"experiment_number" pair
  ...
  def asynchronous_command( command_line )
    #----@client is an object of class ExpCtrlClientNoneDriver which
    #    inherits from ExpCtrl class defined in lib/rctrl/rctrl_service.rb
    response = @client.asynchronous_command( command_line )
    if @experiment_number then
      #----add a new command to the current experiment
      add_command( @experiment_number, response["command_number"] )
    else
      @commands_mutex.synchronize { @commands.push( response["command_number"] ) }
    end
  ...
  def ruby_asynchronous_command( ruby_script )
    #----here response is just a command # - nothing is executed
  ...
  def close_experiment
    ...
    #----get all the information about the command
      ...
      #----here we're actually wait while the cmd finishes its execution
